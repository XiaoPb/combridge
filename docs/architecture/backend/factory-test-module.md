...
### 6.4 RPC receive isolation

RPC 接收路径对产测期间可能交错到达的响应做隔离处理：

- 非安全 RPC 按 key 保存独立的多帧缓冲，因此不同 key 的分片可以交错到达并分别重组，不会互相污染。
- 已成功解析但在语义处理阶段失败的帧，只清理该帧所属 key 的多帧缓冲；其他 key 的接收状态保留。
- parser 或 CRC 错误无法可靠归属到某个 key 时，清空所有 RPC 多帧重组缓冲并进入全局重同步策略。
- `process` 通过接收锁串行化解析与分发，保证并发调用仍按到达顺序处理，避免同一接收状态被并发修改。
- 非安全帧只有在 key 可路由到已注册处理器或待处理动态 call 时才会创建/追加缓冲；不可路由 key 直接忽略，不得建立接收状态。
- 单帧由协议帧大小上限约束（默认 `GHRPC_FRAME_SIZE = 240` 字节）。多帧最多使用 255 个非终止分片（索引 0~254），终止帧使用保留的动态索引标记；累计 payload 上限按 key 的帧开销计算为 `intermediate_payload × 255 + final_payload`，超限或丢帧立即清理该 key 的缓冲。
- 动态 `call` 在收到完整响应后移除 pending 项；发送失败、超时或正常结束也会清理对应动态 call，并清理该 key 的接收缓冲，避免过期状态影响后续产测步骤。
- secure sender/secure RPC 发送流程保持不变；上述按 key 的接收隔离不改变安全发送帧及其重试语义。

GH3036 产测对 `FG`（F_GetMode）结果另有输入校验：空响应直接拒绝并报告“产测模式结果为空”，不会被当作成功结果继续执行。
